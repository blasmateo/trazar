...
pub fn ejecutar(ruta_base: &Path, argumento: Option<&str>) -> Result<(), String> {
    ...
    loop {
        ...
        for campo in campos {
			...
			let valor_str = formatear_valor(valor_actual);
        ...
        println!("\n▸ {}", etiqueta);
        println!("Valor actual: {}", formatear_valor(datos.get(*campo_nombre).unwrap_or(&Value::Null)));
        print!("Nuevo valor (Enter para mantener actual): ");
        
		let valor = match tipo {
			"str" => {
				if *campo_nombre == "nombre" {
					preguntar_nombre_curso(&mut rl, false, &ruta_cursos, Some(id_curso))?
				} else {
					preguntar_texto_con_valor_actual(&mut rl, datos.get(*campo_nombre))?
				}
			}
			"int" => preguntar_numero_con_valor_actual(&mut rl, datos.get(*campo_nombre))?,
			"float" => preguntar_float_con_valor_actual(&mut rl, datos.get(*campo_nombre))?,
			"fecha" => preguntar_fecha_con_valor_actual(&mut rl, datos.get(*campo_nombre))?,
			"enum_int" => {
				let opciones = campo_config["opciones"].as_object()
					.ok_or("Faltan opciones para enum")?;
				preguntar_enum_con_valor_actual(&mut rl, opciones, datos.get(*campo_nombre))?
			}
			"array_str" => {
				let min_items = campo_config["min_items"].as_u64().unwrap_or(0) as usize;
				preguntar_array_con_valor_actual(&mut rl, datos.get(*campo_nombre), min_items)?
			}
			_ => return Err(format!("Tipo desconocido: {}", tipo)),
		};

		// Actualizar valor (el préstamo inmutable de datos.get() ya terminó)
...
fn preguntar_texto_con_valor_actual(rl: &mut DefaultEditor, valor_actual: Option<&Value>) -> Result<Value, String> {
...
fn preguntar_numero_con_valor_actual(rl: &mut DefaultEditor, valor_actual: Option<&Value>) -> Result<Value, String> {
    let input = rl.readline("> ")
        .map_err(|e| format!("Error al leer entrada: {}", e))?;
    ...
    match valor.parse::<i64>() {
        ...
        Err(_) => {
            println!("Debe ingresar un número válido. Se mantiene el valor actual.");
            Ok(valor_actual.cloned().unwrap_or(Value::Null))
        }
    }
}

fn preguntar_float_con_valor_actual(rl: &mut DefaultEditor, valor_actual: Option<&Value>) -> Result<Value, String> {
    let input = rl.readline("> ")
        .map_err(|e| format!("Error al leer entrada: {}", e))?;
...
fn preguntar_fecha_con_valor_actual(rl: &mut DefaultEditor, valor_actual: Option<&Value>) -> Result<Value, String> {
    let input = rl.readline("> ")
        .map_err(|e| format!("Error al leer entrada: {}", e))?;
...
fn preguntar_array_con_valor_actual(
    ...
    println!("Ingrese uno por línea (deje vacío para terminar, o Enter inmediato para mantener actual):");
